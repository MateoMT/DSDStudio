...
namespace Solver
{
            ...
            int debugPreviewCount = 3)
        {
            ...
            while (t < tfinal - 1e-14)
            {
                ...
                while (!stepAccepted)
                {
                    ...
                    bool invalidStep = !double.IsFinite(errorNorm) || !double.IsFinite(suggestedDt) || HasNonFinite(yNext);

                    // 计算新步长
                    double newStep = dtAttempt;
                    if (invalidStep)
                    {
                        newStep = Math.Max(0.1 * dtAttempt, minStep);
                    }
                    else if (errorNorm > 1e-15)
                    {
                    ...
                    if (debugOutput && (times.Count - 1) % debugEvery == 0 && (trialIndex <= 10 || trialIndex % debugEvery == 0))
                    {
                    ...
                    {
                        throw new InvalidOperationException($"RK45 exceeded {maxStepTrials} trials at t={t:F6}, dt={dtAttempt:E6}, errNorm={errorNorm:E6}. The ODE system may be stiff or numerically unstable.");
                    }
                    ...
                    if (invalidStep)
                    {
                        if (dtAttempt <= minStep)
                        {
                            throw new InvalidOperationException($"RK45 produced non-finite values at t={t:F6} with minStep={minStep:E3}. Try smaller initial concentrations, lower reaction rates, or a less explosive random system.");
                        }
    ...
    double tol = 1e-6, double dtMin = 1e-10, double dtMax = 1.0)
            ...
            int debugPreviewCount = 3)//根据物质数目选择调用GPU还是CPU求解，并返回一致的结果
        {
            ...
            {
                (times,results) = solver.Solve(initialConcentrations, tStart, tEnd, dt, rtol, atol, safetyFactor, minStep, debugOutput, debugEvery, debugPreviewCount);
            }
            return (times, results);
        }
