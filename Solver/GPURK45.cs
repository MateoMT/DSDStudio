...
namespace Solver
{
    public class GPURK45 : IDisposable
    {
        ...
        public GPURK45(ODE.ODEsys system)
        {
            ...
            Accelerator? selectedAccelerator = null;
            foreach (Device d in this.context.GetCudaDevices())//先选择CUDA
            {
                selectedAccelerator = d.CreateAccelerator(this.context);
                //Console.WriteLine(accelerator);
            }
            if (selectedAccelerator == null)
            {
                foreach (Device d in this.context.GetCLDevices())//如果没有CUDA设备，则选择OpenCL，但是集显可能不支持float64
                {
                    selectedAccelerator = d.CreateAccelerator(this.context);
                    //Console.WriteLine(accelerator);
                }
            ...
            if (selectedAccelerator == null)
            {
                Console.WriteLine("No available GPU device found.");
                throw new InvalidOperationException("No available GPU device found.");
            }
            this.accelerator = selectedAccelerator;
            ...
            while (t < tEnd)
            {
                ...
                while (!stepAccepted)
                {
                    ...
                    calculateRatesKernel(gpuData.SubstanceCount,
                        ...
                        d_equationTermCount.View, d_concentrations.View, gpuData.SubstanceCount, 0, trialDt);
                    ...
                    computeTempStateKernel(gpuData.SubstanceCount,
                        ...
                        d_a_flattened.View, trialDt, 1, gpuData.SubstanceCount);
                    // 使用临时状态计算k2
                    calculateRatesKernel(gpuData.SubstanceCount,
                        d_concentrations.View, d_k2.View,
                        d_termCoefficients.View, d_termFactorsStart.View, d_termFactorsLength.View,
                        d_factorSubstanceIds.View, d_factorExponents.View, d_equationTermStart.View,
                        d_equationTermCount.View, d_tempState.View, gpuData.SubstanceCount, 1, trialDt);
                    //Console.WriteLine("k2计算完成");

                    computeTempStateKernel(gpuData.SubstanceCount,
                        d_concentrations.View, d_tempState.View,
                        d_k1.View, d_k2.View, d_k3.View, d_k4.View, d_k5.View,
                        d_a_flattened.View, trialDt, 2, gpuData.SubstanceCount);
                    ...
                    computeTempStateKernel(gpuData.SubstanceCount,
                        d_concentrations.View, d_tempState.View,
                        d_k1.View, d_k2.View, d_k3.View, d_k4.View, d_k5.View,
                        d_a_flattened.View, trialDt, 3, gpuData.SubstanceCount);
                    ...
                    computeTempStateKernel(gpuData.SubstanceCount,
                        d_concentrations.View, d_tempState.View,
                        d_k1.View, d_k2.View, d_k3.View, d_k4.View, d_k5.View,
                        d_a_flattened.View, trialDt, 4, gpuData.SubstanceCount);
                    ...
                    computeTempStateKernel(gpuData.SubstanceCount,
                        d_concentrations.View, d_tempState.View,
                        d_k1.View, d_k2.View, d_k3.View, d_k4.View, d_k5.View,
                        d_a_flattened.View, trialDt, 5, gpuData.SubstanceCount);
                    ...
                    // 计算最大归一化误差（混合容差: atol + rtol * max(|y_n|, |y_{n+1}|)）
                    error = 0.0;
                    for (int i = 0; i < gpuData.SubstanceCount; i++)
                    {
                        ...
                        yNext[i] = currentState[i] + trialDt * (
                            b5[0] * k1[i] + b5[1] * k2[i] + b5[2] * k3[i] +
                            b5[3] * k4[i] + b5[4] * k5[i] + b5[5] * k6[i]
                        );

                        double scale = atol + rtol * Math.Max(Math.Abs(currentState[i]), Math.Abs(yNext[i]));
                        double normalizedErr = yErr[i] / scale;
                        error = Math.Max(error, normalizedErr);
                    }


                    //Console.WriteLine($"误差计算完成{error}");
                    double newStep = trialDt;
                    bool invalidStep = !double.IsFinite(error) || HasNonFinite(yNext);
                    if (invalidStep)
                    {
                        newStep = Math.Max(0.1 * trialDt, minStep);
                    }
                    else if (error > 0)
                    {
                        newStep = safetyFactor * trialDt * Math.Pow(1.0 / error, 0.2);
                        newStep = Math.Max(newStep, 0.1 * trialDt);
                        newStep = Math.Min(newStep, 5.0 * trialDt);
                    }

                    if (debugOutput && acceptedStepIndex % debugEvery == 0 && (trialIndex <= 10 || trialIndex % debugEvery == 0))
                    {
                        Console.WriteLine($"[RK45-TRIAL] step={acceptedStepIndex + 1}, trial={trialIndex}, t={t:F6}, dt={trialDt:E6}, errNorm={error:E6}, nextDt={newStep:E6}");
                    }

                    if (trialIndex > maxStepTrials)
                    {
                        throw new InvalidOperationException($"GPU RK45 exceeded {maxStepTrials} trials at t={t:F6}, dt={trialDt:E6}, errNorm={error:E6}. The ODE system may be stiff or numerically unstable.");
                    }
                    ...
                    if (invalidStep)
                    {
                        if (trialDt <= minStep)
                        {
                            throw new InvalidOperationException($"GPU RK45 produced non-finite values at t={t:F6} with minStep={minStep:E3}. Try smaller initial concentrations, lower reaction rates, or a less explosive random system.");
                        }
                    ...
                    bool forceAcceptAtMinStep = error > 1.0 && trialDt <= minStep;
                    if (error <= 1.0 || forceAcceptAtMinStep)
                    {
                        if (forceAcceptAtMinStep && !minStepWarningShown)
                        {
                            Console.WriteLine($"Warning: Minimum step size {minStep:E3} reached at t={t:F6}, but normalized error {error:E3} still exceeds 1 (rtol={rtol:E3}, atol={atol:E3}). Continuing integration at minimum step size.");
                        ...
                        if (debugOutput && acceptedStepIndex % debugEvery == 0)
                        {
                            int previewCount = Math.Min(debugPreviewCount, currentState.Length);
                            string preview = string.Join(", ", currentState.Take(previewCount).Select(v => v.ToString("E6")));
                            Console.WriteLine($"[RK45-ACCEPT] step={acceptedStepIndex}, t={t:F6}, dtUsed={trialDt:E6}, errNorm={error:E6}, y[0..{previewCount - 1}]={preview}");
                        }
