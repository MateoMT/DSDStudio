    ...
    public class Parser
    {
        ...
        private bool MatchRightBracket()
        {
            if (MatchOperator(Operator.RightBracket))
            {
                ...
                {
                    AddError($"Mismatched ']' bracket, {stack.Count} unclosed");
        ...
        private void AddError(string msg)
        {
            ...
            throw new Exception($"Line {error.line}, Column {error.column}: {error.message}");
        }
                ...
                switch ((Keyword)Current.Value)
                {
                    ...
                    case Keyword.lengths:
                        ...
                        if (list1.Values.Count != 2)
                            AddError("Invalid 'lengths' parameter");
                    ...
                    case Keyword.toeholds:
                        ...
                        if (list2.Values.Count != 2)
                            AddError("Invalid 'toeholds' parameter");
                    ...
                    case Keyword.compilation:
                        ...
                        if (!Match(TokenType.Keyword))
                        {
                            AddError("Invalid 'compilation' parameter");
                            break;
                        }
                    ...
                    case Keyword.simulator:
                        ...
                        if (!Match(TokenType.Keyword))
                        {
                            AddError("Invalid 'simulator' parameter");
                            break;
                        }
                        directive.Value = GetFromCurrent();
                        Next();
        ...
        private DeclareNode ParseDom()
        {
                ...
                if (MatchEqual())
                {
                    ...
                    if (MatchLeftBrace())
                    {
                        ...
                        foreach (var para in paras.Values)
                        {
                            ...
                            {
                                AddError("Invalid 'dom' parameter (expected assignment)");
                                return null;
                            }
                            ...
                            if(binary.Left is not NameNode || binary.Right is not ValueNode)
                            {
                                ...
                                AddError("Invalid 'dom' parameter (expected name=value)");
                                return null;
                            }
                            ...
                            switch (nd.GetValue())
                            {
                                ...
                                case "colour":
                                    ...
                                    break;
                                default:
                                    AddError("Invalid parameter for 'dom'");
                                    return null;
                            }
                        }
                        declare.Value = dom;
            ...
            {
                AddError("Invalid domain definition in 'dom'");
                return null;
            }
            return declare;
        }
                ...
                if(!Match(TokenType.Name))//虽然是个名字，但其实是个函数
                {
                    if(MatchOperator(Operator.Star))
                    {
                        ...
                        if(!Match(TokenType.Name))//后面是复合物
                        {
                            ...
                            {
                                AddError("Expected species name");
                                return null;
                            }
                        }
                        else
        ...
        private ExpressionNode ParseBaseSpecies()//用来解析plots里单独的一个物种,不吃掉分隔符
        {
            ...
            else if (Match(TokenType.Integer) || Match(TokenType.Float))
            {
                ...
                if (!MatchMathOperator())
                {
                    AddError("Expected arithmetic operator");
                    return null;
                }
                binaryExpression.Operator = (Operator)Current.Value;
                Next();
        ...
        private StrandNode ParseStrand()//遇到<{[调用的,会吃掉后面的符号，但不会吃掉分隔符,;
        {
            ...
            {
                AddError("Invalid strand segment");
                return null;
            }
            strand.seq = ParseSeq();
        ...
        private ValueNode GetFromCurrent()//从当前Token得到一个新的ValueNode,除了函数以外不next
        {
            ...
            AddError("Invalid current value");
            return null;
        }
    }
}
