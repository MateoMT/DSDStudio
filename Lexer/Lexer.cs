        ...
        private Dictionary<string, Units> _units = new Dictionary<string, Units>
{
    ...
    {"yM", Units.yM}
        ...
        public Token NextToken()
        {
            ...
            Console.WriteLine($"Unexpected character: {Current} at Line {_line}, Column {_column}");
            //throw new Exception($"Unexpected character: {Current} at Line {_line}, Column {_column}");
            AddError($"Unexpected character: {Current}");
            return new Token(TokenType.EOF, "EOF", _line, _column);
        ...
        private Token ReadNumber()
        {
            ...
            if (Current == 'e' || Current == 'E')
            {
                ...
                if (!char.IsDigit(Current))
                {
                    ...
                    AddError("Invalid scientific notation");
                }
                while (char.IsDigit(Current))
                {
                    _position++;
        ...
        private Token ReadNameAndKey()
        {
            ...
            if (!Regex.IsMatch(name, pattern))
            {
                ...
                Console.WriteLine($"Unexpected string: {Current} at Line {_line}, Column {column}");
                //throw new Exception("Invalid identifier name");
                AddError("Invalid identifier name");
            }
        ...
        private Token ReadString()
        {
            ...
            while (Current != quote)
            {
                if (Current == '\0')
                {
                    ...
                    AddError("Invalid string literal");
                }
                _position++;
            }
            _position++;
