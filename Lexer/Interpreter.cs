    ...
    internal class Interpreter//用于解释执行
    {
            ...
            switch (complex)
            {
                case DomNode dom:
                    ...
                    if (variableContext.variables.ContainsKey(name))
                    {
                        ...
                        {
                            errors.AddError(dom.Line, dom.Column, "Domain variable is not a domain");
                        }
                    }
                    else
                    {
        ...
        private bool CollectVariables(StatementNode statement)//收集变量 def和dom以及new产生的
        {
            ...
            if (declare.Name is FuncNode)
            {
                ...
                {
                    errors.AddError(func.Line, func.Column, $"'{name_}' is already defined");
                    return false;
                }
            ...
            switch (value)
            {
                case DomNode dom:
                    if (variableContext.variables.ContainsKey(name.Name))
                    {
                        errors.AddError(dom.Line, dom.Column, "Duplicate definition");
                        return false;
                    }
                    if (dom.colour != null)
                    {
                ...
                case ValueNode val:
                    if (variableContext.variables.ContainsKey(name.Name))
                    {
                        errors.AddError(val.Line, val.Column, "Duplicate definition");
                        return false;
                    }
                    variableContext.id.Add(name.Name);
        ...
        private bool CollectSpecies(StatementNode statement)//这里就是查找process里的了
        {
            ...
            switch (proces.Value)
            {
                ...
                case Species species://对于单个的DNA复合物或者是函数调用 val1 数目 val2 加入的时间 name
                    ...
                    {
                        errors.AddError(value1.Line, value1.Column, "Expected a numeric value");
                        return false;
                    }
                    ...
                    else if (value1 is NameNode)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                errors.AddError(value1.Line, value1.Column, $"Variable '{(string)(name.GetValue())}' does not exist");
                                return false;
                            }
                            if (variableContext.variables[(string)(name.GetValue())] is not (IntegerNode or FloatNode))
                            {
                                errors.AddError(value1.Line, value1.Column, $"Variable '{(string)(name.GetValue())}' must be integer or float");
                                return false;
                            }
                    ...
                    else
                    {
                        ...
                        {
                            errors.AddError(value2.Line, value2.Column, "Expected a numeric value");
                            return false;
                        }
                        ...
                        else if (value2 is NameNode)
                        {
                            ...
                            else
                            {
                                ...
                                if (!variableContext.variables.ContainsKey((string)(name.GetValue())))
                                {
                                    errors.AddError(value2.Line, value2.Column, $"Variable '{(string)(name.GetValue())}' does not exist");
                                    return false;
                                }
                                if (variableContext.variables[(string)(name.GetValue())] is not (IntegerNode or FloatNode))
                                {
                                    errors.AddError(value2.Line, value2.Column, $"Variable '{(string)(name.GetValue())}' must be integer");
                                    return false;
                                }
                                if (variableContext.variables[(string)(name.GetValue())] is FloatNode)
                                    pro.time = (int)((FloatNode)value2).GetValue();
                    ...
                    switch (species.Name)
                    {
                        case FuncNode func://需要调用函数来生成物种
                            ...
                            if (!variableContext.variables.ContainsKey(name))
                            {
                                errors.AddError(func.Line, func.Column, $"Function '{name}' does not exist");
                                return false;
                            }
                            ...
                            {
                                errors.AddError(func.Line, func.Column, $"Function '{name}' argument count mismatch");
                                return false;
                            }
                            ...
                            if (id1 == -1)
                            {
                                errors.AddError(func.Line, func.Column, "Invalid species definition");
                                return false;
                            }
                            pro.id = id1;
                            break;
                        ...
                        case BaseComplex complex:
                            ...
                            if (id == -1)
                            {
                                errors.AddError(species.Line, species.Column, "Invalid species definition");
                                return false;
                            }
                            pro.id = id;
                            break;
                ...
                case BaseComplex complex://直接加入物种
                    ...
                    if (id2 == -1)
                    {
                        errors.AddError(complex.Line, complex.Column, "Invalid species definition");
                        return false;
                    }
                    pro.id = id2;
                    break;
        ...
        private BaseComplex FuncCall(List<NameNode> funcA, List<NameNode> actualA, AstNode expressions)//完成函数调用,得能完成函数套函数的递归操作
        {
            ...
            {
                errors.AddError(0, 0, "Function body is empty");
                return null;
            }
            if (funcA is not null && actualA is not null && funcA.Count != actualA.Count)
            {
                errors.AddError(expressions.Line, expressions.Column, "Argument count mismatch");
                return null;
            }
            ...
            switch (expressions)//目前假设函数体就是一个复合物，不会函数嵌套函数。实际上还有可能是一个ProcessList
            {
                ...
                case StrandNode strand_://是一个简单链
                    ...
                    if (seq is null)
                    {
                        errors.AddError(expressions.Line, expressions.Column, "Strand sequence is empty");
                        return null;
                    }
                    ...
                    if (seq.Value is List<SeqNode>)//是一堆域，需要对每个域进行替换
                    {
                        ...
                        for (int i = 0; i < list.Count; i++)
                        {
                            ...
                            {
                                errors.AddError(list[i].Line, list[i].Column, "Strand must contain domains");
                                return null;
                            }
                    ...
                    {
                        errors.AddError(seq.Line, seq.Column, "Invalid strand sequence");
                        return null;
                ...
                case ComplexNode complex://是一个复合物
                    ...
                    {
                        errors.AddError(complex.Line, complex.Column, "Invalid complex definition");
                        return null;
                    }
        ...
        private strandInfos getSubStrand(StrandNode strand)//获取子链们,按照起点长度获取
        {
            ...
            {
                errors.AddError(strand.Line, strand.Column, "Expected strand of domain list");
                return infos;
            }
        ...
        private bool checkMatch(StrandNode s1, StrandNode s2)//从0开始
        {
            ...
            {
                errors.AddError(s1.Line, s1.Column, "Strand must be a sequence");
                return false;
            }
            ...
            {
                errors.AddError(s1.Line, s1.Column, "Strand lengths are inconsistent");
                return false;
            }
        ...
        private ComplexNode Stickss(StrandNode strand1, StrandNode strand2, matchInfo info)//把两个链粘在一起，假设第一个是上链，第二个是下链
        {
            ...
            {
                errors.AddError(s1.Line, s1.Column, "Position + length exceeds strand length");
                return null;
            }
            //可能是一个契合的双链
            ...
            foreach(var item in plots)
            {
                if(item is FuncNode func)
                {
                    ...
                    if(func.Expressions!=null && func.Expressions is BaseComplex complex)
                    {
                        ...
                        {
                            errors.AddError(func.Line,func.Column,"Complex does not meet requirements");
                            return false;
                        }
                ...
                else if(item is ComplexNode complex)
                {
                    ...
                    if (complex is ComplexNode node && !checkComplex(node, false))
                    {
                        errors.AddError(complex.Line, complex.Column, "Complex does not meet requirements");
                        return false;
                    }
                }
            }
